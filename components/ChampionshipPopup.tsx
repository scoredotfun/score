...
const BetPopup: React.FC<BetPopupProps> = ({ team, odds, onClose }) => {
    ...
    const [selectedBookmaker, setSelectedBookmaker] = useState<string>(Object.keys(odds)[0]);
    ...
    const handleBet = () => {
        ...
        console.log(`Placing bet of ${amount} $SCORE on ${team} with ${selectedBookmaker}`);
        setShowSuccess(true);
        setTimeout(() => {
                ...
                <div className="relative z-10">
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            ...
                            >
                                {Object.entries(odds).map(([bookmaker, odds]) => (
                                    <option key={bookmaker} value={bookmaker}>
                                        {bookmaker.charAt(0).toUpperCase() + bookmaker.slice(1)} ({odds > 0 ? '+' : ''}{odds})
                                    </option>
                                ))}
                            </select>
                        </div>
                ...
                <div className="fixed inset-0 z-70 flex items-center justify-center bg-black/80 backdrop-blur-sm animate-fadeIn">
                    <div className="bg-gray-900 rounded-2xl p-8 text-center relative overflow-hidden">
                        ...
                        <div className="relative z-10">
                            ...
                            <p className="text-gray-400">Your bet has been confirmed on the Solana network</p>
                        </div>
                    </div>
                </div>
            )}
...
const ChampionshipPopup: React.FC<PopupProps> = ({ onClose, selectedLeague }) => {
    const apiKey = "960601334be61607620059689f8a77b1";
    const [winningOdds, setWinningOdds] = useState<TeamOdds[]>([]);
    const [loading, setLoading] = useState<boolean>(false);
    const [error, setError] = useState<string | null>(null);
    const [aiPrediction, setAiPrediction] = useState<{ team: string; reason: string } | null>(null);
    const [selectedBet, setSelectedBet] = useState<TeamOdds | null>(null);

    // Helper to format odds (prepend a plus for positive values)
    const formatOdds = (price: number | undefined): string => {
        if (price === undefined) return "-";
        return price > 0 ? `+${price}` : `${price}`;
    };

    // Helper to compute composite winning odds (average of provided odds)
    ...
    useEffect(() => {
        async function fetchWinningOdds() {
            ...
            setError(null);
            try {
                const selectedBookmakers = ["draftkings", "fanduel", "betmgm", "unibet"];
                ...
                if (Array.isArray(data) && data.length > 0) {
                    const event = data[0];
                    let oddsTable: Record<string, Record<string, number>> = {};
                    ...
                    const tableData: TeamOdds[] = Object.entries(oddsTable).map(([team, odds]) => ({
                        team,
                        odds,
                    }));
                    ...
                    if (tableData.length > 0) {
                        const bestTeam = tableData.reduce((prev, curr) => {
                            return computeCompositeOdds(curr.odds) < computeCompositeOdds(prev.odds)
                                ? curr
                                : prev;
                        });
                        setAiPrediction({
                ...
                const message = err instanceof Error ? err.message : "Unknown error fetching odds.";
                setError(message);
            } finally {
                setLoading(false);
            }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-80 backdrop-blur-sm">
            <div className="relative bg-gray-900 text-white rounded-2xl shadow-2xl w-4/5 max-h-[80vh] overflow-hidden">
                ...
                <div className="sticky top-0 z-20 bg-gray-900 border-b border-gray-700 flex justify-between items-center px-6 py-4">
                    <button onClick={onClose} className="text-lg text-green-400 hover:text-green-300 transition-colors">
                        &larr; Back
                    </button>
                    ...
                    <button onClick={onClose} aria-label="Close" className="text-4xl font-bold text-green-400 hover:text-green-300 transition-colors">
                        &times;
                    </button>
                </div>

                {/* Main Content Area with scrollable design and custom scrollbar */}
                            ...
                            <div className="relative">
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <div className="text-xl font-bold text-white">{aiPrediction.team}</div>
                            ...
                            <table className="min-w-full border-collapse">
                                <thead>
                                    <tr className="border-b border-gray-700">
                                        <th className="px-6 py-3 text-left text-gray-400">Winning Odds</th>
                                        {["draftkings", "fanduel", "betmgm", "unibet"].map(bookmaker => (
                                            <th key={bookmaker} className="px-6 py-3 text-left text-gray-400">
                                                <div className="flex items-center gap-2">
                                                    <div className="relative">
                                                        <img
                                                            src={`/images/${bookmaker === 'draftkings' ? 'dkinglogo' : bookmaker}.png`}
                                                            alt={bookmaker}
                                                    ...
                                                    <span>{bookmaker.charAt(0).toUpperCase() + bookmaker.slice(1)}</span>
                                                </div>
                                            </th>
                                        ))}
                                    </tr>
                                                ...
                                                {["draftkings", "fanduel", "betmgm", "unibet"].map(bookmaker => (
                                                    <td key={bookmaker} className="px-6 py-4 text-xl">
                                                        <div className="relative overflow-hidden rounded-lg group/cell">
                                                            <div className="absolute inset-0 bg-gradient-to-r from-green-400/10 to-transparent opacity-0 group-hover/cell:opacity-100 transition-opacity" />
                                                            <div className="relative z-10 text-center">
                                                                <div className="text-white font-medium">{formatOdds(row.odds[bookmaker])}</div>
